...
% Output: spessore espresso in unita'  intere [voxel]
% la funzione e' basata sull'algoritmo di Brasenham per la circonferenza
...
for i=1:R_max 
    ...
    while dz<=dy %scorre i punti della circonferenza del primo ottante superiore
            ...
            %e' sufficiente che un solo punto della circonferenza appartenga alla trabecola 
            %per uscire dal while e incrementare il raggio di analisi
        
            if  yc+dy>=1 && yc+dy<=dim && zc+dz>=1 && zc+dz<=dim && (mesh_modificata(xc,yc+dy,zc+dz) == numero_cricca) % || mesh_modificata(xc,yc+dy,zc+dz) == ID_trabecola+1)
                logical=0;
                break
            end
            
            if  yc+dz>=1 && yc+dz<=dim && zc+dy>=1 && zc+dy<=dim && (mesh_modificata(xc,yc+dz,zc+dy) == numero_cricca) % || mesh_modificata(xc,yc+dz,zc+dy) == ID_trabecola+1)
                logical=0;
                break
            end
            
            if  yc-dy>=1 && yc-dy<=dim && zc+dz>=1 && zc+dz<=dim && (mesh_modificata(xc,yc-dy,zc+dz) == numero_cricca) 
                logical=0;
                break
            end
            
            if  yc-dz>=1 && yc-dz<=dim && zc+dy>=1 && zc+dy<=dim && (mesh_modificata(xc,yc-dz,zc+dy) == numero_cricca) 
                logical=0;
                break
            end
    ...
    %se logical e' rimasto uno significa che il while ha scorso tutta la
    %circonferenza senza incontrare punti appartenenti alla trabecola.
    %di conseguenza il raggio e' quello cercato e si esce dal for,
